...
classdef SafeCoExpressSorting < handle
    ...
    methods
        ...
        function SetupEnvironment(start,initpos,spawn)
            ...
            [f,v,data] = plyread('rcup.ply','tri'); %%Import Stand
            vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255; 
            for XSpacing = [0,0]
                for YSpacing = [0,0]
                    trisurf(f,v(:,1)+ XSpacing-0.6,v(:,2) + YSpacing-0.6 , v(:,3)+ ZSpacing ... 
                        ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
                end
            end
            
            [f,v,data] = plyread('gcup.ply','tri'); %%Import Stand
            vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255; 
            for XSpacing = [0,0]
                for YSpacing = [0,0]
                    trisurf(f,v(:,1)+ XSpacing-0.6,v(:,2) + YSpacing-0.8 , v(:,3)+ ZSpacing ... 
                        ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
                end
            end
            
            [f,v,data] = plyread('bcup.ply','tri'); %%Import Stand
            vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255; 
            for XSpacing = [0,0]
                for YSpacing = [0,0]
                    trisurf(f,v(:,1)+ XSpacing-0.6,v(:,2) + YSpacing-1.2 , v(:,3)+ ZSpacing ... 
                        ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
                end
            end
            ...
            for XSpacing = [0,0]
                for YSpacing = [0,0]
                    trisurf(f,v(:,1)+ XSpacing,v(:,2) + YSpacing -1 , v(:,3)+ ZSpacing-0.5 ... 
                        ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
                end
            end
